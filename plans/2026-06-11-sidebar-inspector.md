...
10. **Zero-with-delta rows stay visible (2026-06-11)**: Review finding 3 — in delta mode, a metric whose value dropped to 0 was classified as empty and hidden in the collapsed group. The empty classification is now `isEmptyMetricRow` (no value AND no delta), so such rows stay in the main list with their delta.
11. **Template split + explicit empty check (2026-06-11)**: The header and metrics-list templates grew too large; extracted four presentational components — `inspectorNodePath`, `inspectorNodeBadges`, `inspectorComparisonToggle`, `inspectorEmptyMetrics` — each with its own spec; all testids preserved. CodeRabbit flagged `isEmptyMetricValue` treating 0 as empty: that is intentional (zero-valued metrics belong in the collapsed Empty group; rows with deltas are protected by `isEmptyMetricRow`), but the check is now written explicitly (`value === undefined || value === 0 || Number.isNaN(value)`) instead of `!value`.
