...
- [ ] **#5 part (b) — `ccsh check` referential integrity** (NOT done). Landed part (a) only warns at read time; `ValidationTool` still does pure JSON-schema validation, so a file with a dangling metrics-lens id or a dangling edge `fromId`/`toId` still **passes `ccsh check`**. Follow-up: add a semantic pass that asserts every lens key and every edge endpoint resolves to a file-tree id (needs its own EveritValidator/ValidationTool tests). Larger than the read warn; kept separate on purpose.
- [x] **#8 — no missing coverage, one deliberate omission.** The golden is fully de-blinded (2.0-vs-legacy structural greps + edge `fromId` + #12 drop-warning + a deterministic ConvertTool unit test pinning lens data survival). The investigation's *alternative* — a committed `convert_gold.cc.json` byte-diff oracle — was **skipped on purpose** to avoid regenerating a gold file on every future convert-output change. Revisit only if a byte-exact convert oracle is wanted.
...
- [ ] dep-cruiser: `options.exclude` drops `mocks/` from the graph entirely (a prod import of a mock would be invisible); blanket home exemptions make cross-home raw-store imports rule-legal; `render-model` facade-only claim unenforced; `app/app.config.ts`/`main.ts` sit outside most fences.
...
- [ ] Tests: `edges.selector.spec` largely tautological (asserts implementation against itself), no delta-mode value case; ~~`accumulatedDataSelector` (new composing layer) has no spec incl. the clone-before-decorate invariant~~ (**done** with #24 — clone-before-decorate + single-pass unique-id + cross-selector mesh-parity); v15 migration lacks a dedicated unit test (chain test title still says v14); stale "from dynamicSettings" titles in 3dPrint specs; `loadFiles()` has no service-level 2.0 case; the single viz 2.0 fixture is hand-written (path ids, fake checksum) — **no fixture produced by ccsh is parsed by any viz test, and vice versa**.
- [ ] Bundled sample files are still 1.2-format — the default first-run never exercises the 2.0 reader.
