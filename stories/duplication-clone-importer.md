...
- **cc.json 2.0 alignment:** the per-file scalar maps to `lenses.metrics.attributes`. The natural home for N-way clone groups is the top-level `clusters` lens, whose shape is defined in [`dev_docs/cc-json-2.0-clusters-lens.md`](../dev_docs/cc-json-2.0-clusters-lens.md) — a clone class is a clustering with `membership: "overlay"`, one cluster per clone class, members joined by node id. 2.0 ships the definition but no producer, so a follow-up can have this importer emit clusters without a format change.
...
- Sibling stories: `cc-json-2-dto.md` (the 2.0 DTO), `software-report-view.md` (consumer), `migrate-dependacharta.md` (the importer/edge template to mirror).
