...
our [Web Studio](https://codecharta.com/visualization/app/index.html?file=codecharta_visualization.cc.json.gz&file=codecharta_analysis.cc.json.gz&area=rloc&height=sonar_complexity&color=sonar_complexity&edge=avgCommits&currentFilesAreSampleFiles=true) which is there to visualize these metrics as a city-like map.
...
You can use
our [Web Studio](https://codecharta.com/visualization/app/index.html?file=codecharta_visualization.cc.json.gz&file=codecharta_analysis.cc.json.gz&area=rloc&height=sonar_complexity&color=sonar_complexity&edge=avgCommits&currentFilesAreSampleFiles=true)
to move around and check out our example maps that are already in place.

### Docker

Both the Web Studio and the CodeCharta Shell are available as docker containers. For more information, see [Docker](/docs/overview/getting-started/docker).

### Installation

To install our [CodeCharta Shell](/docs/analysis/codecharta-shell), which is used to generate the code maps, simply install it via npm:
...
If you want to analyze your own project, [Getting Started](/docs/overview/getting-started) is a great place to start.
