---
title: "Examples"
description: "How to quickly install and setup CodeCharta."
---

## Generate Metrics from Code

To analyze projects, the ccsh provides the [Unified Parser](/docs/parser/unified), which supports multiple programming languages.
...
The ccsh provides the [Tokei Importer](/docs/importer/tokei), which allows import of metrics generated by [Tokei](https://github.com/XAMPPRocky/tokei#installation). Tokei supports over 200 languages but only computes basic metrics.
...
SonarQube is a popular platform to analyze code in a variety of metrics. To use it with CodeCharta, you first need to install [SonarQube](https://www.sonarqube.org/downloads). After, you can analyze your project as described [here](/docs/analysis/example-sonar-qube).
...
This part requires you to have [Git](https://git-scm.com/downloads) installed and to already have a `cc.json` file, e.g. from one of the previous sections of this guide. Now, in case the code you analyzed used git, the previously generated metrics will be combined with data from git logs. If you analyzed the code of [JUnit4](https://github.com/junit-team/junit4), executing the [Git Log Parser](/docs/parser/git-log) would look like this:

```bash
# Generate and parse a <project>.git.log
cd junit4; ccsh gitlogparser repo-scan --repo-path . -o junit4.git.cc.json -nc
```

We can now merge this with the previously generated file by using the [Merge Filter](/docs/filter/merge-filter).
...
CodeCharta is not limited to the importers mentioned here. The [analysis docs](/docs/analysis/codecharta-shell) show the way. They are also worth checking out if you need some custom metrics. Furthermore, you should read the [visualization docs](/docs/visualization/web-studio) to find out more about the available features.
