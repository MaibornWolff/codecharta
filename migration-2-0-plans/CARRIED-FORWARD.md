...
| 2 | **Dependency (edge) lens** — move `calculateEdgeMetricData`, `metricNamesSelector` (edge names), `sortedNodeEdgeMetricsMap`, and the **edge side of `attributeTypes`** out of `state/` (`attributeDescriptors` is a **flat** metric→descriptor map with no node/edge split — migrate or share it wholesale when the edge lens needs it, don't try to "split" it); shrink `metricDataSelector` to nothing once its edge/cross-cutting consumers migrate. | Slices 1 & 2 (node side only) | Not started; edges intentionally left in place. | **Dependency lens** slice | — |
