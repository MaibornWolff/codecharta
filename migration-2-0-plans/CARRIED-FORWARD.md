...
| 2b | **Edge *selection* + edge appearance + the 3 edge effects** — `dynamicSettings.edgeMetric` (selection) and the edge appSettings (`isEdgeMetricVisible`, `amountOfEdgePreviews`, `edgeHeight`, `showIncoming/OutgoingEdges`, `showOnlyBuildingsWithEdges`, edge `mapColors`), plus the effects that read/write them (`updateEdgePreviews`, `updateAmountOfEdgePreviews`, `resetSelectedEdgeMetricWhenItDoesntExistAnymore`). The dependency lens owns edge *data*, not its *selection*/*appearance*. | Slice 3 scope guard | Per-view config/appearance, same verdict `colorMetric` got; effects read/write it. | **viewState/appearance** slice | those modules exist. |
