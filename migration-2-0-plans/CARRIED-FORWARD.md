...
|---|------|---------------|------------------|--------------|-------------------|
| 1 | **`valueOf(id, metric)` — cycle-free per-node lookup DONE (Slice 14d); lens-native promotion → 14e.** Slice 14d added `valueOfSelector` (`state/selectors/accumulatedData/valueOf.selector.ts`) in the **composing layer** above the lenses: it resolves a node by id against the decorated `idToNode` and reads the metric off its aggregated attributes. Placing it above the lenses (where `new-must-not-import-legacy`@error forbids `lenses/ → state/`, so no lens can import it back) makes it cycle-free — resolving the CF#1 cycle. **Still deferred to 14e:** moving `valueOf` ONTO the `MetricsLensFacade` (needs the lens's own id-keyed attributes = the 2.0 PATH id surviving load) + moving `idToNode` ownership into the structure lens (same PATH-id prerequisite; a lens can't own a decoration-independent id→node until the PATH id, which is assigned pre-decoration). | Slice 2 (was a DoD item); cycle-free form landed Slice 14d | Node ids are still the decoration-time ordinal (`NodeDecorator.decorateMapWithMetricData`); a **lens-native** `valueOf` needs a decoration-independent id keyed on the lens's own attributes. The `blacklist-classify ↔ mergeFolderChain ↔ id` ordering (and `lens-no-view-state` banning a blacklist read in the lens) blocks a metric-free structure pass until the PATH id exists. | **14e** (renderer-agnostic PATH id) | The PATH id survives load and keys the metrics lens's own attributes, so the lens exposes per-node values without walking the downstream-decorated tree. |
...
| 10 | **Move the initial-file loader kickoff into `load/`.** Today `fileStore/loaders/ccJson/services/loadInitialFile.service.ts` imports `LoadInitialFileStore` from `load/` — the single deliberately-allowed `fileStore → load/` edge (the ingestion boundary *kicks off* the top-layer load orchestrator). Lift that kickoff seam UP into `load/` so `fileStore` has **zero upward deps** and `load/` is a clean top layer that fileStore's ingestion simply hands off to. NB: the orchestrator (`LoadInitialFileStore`) itself must **stay** in `load/` — it drives the homes + both lens load-facades + fileStore, so it can never live in `fileStore` (`filestore-has-no-upward-deps` forbids fileStore→lenses/mapState). This is the loader→load move, NOT an orchestrator→fileStore move. | Slice 12b (the `load-orchestrator-not-imported-by-lower-layers` rule comment flags it: "a follow-up may move the loader itself into `load/`") | Nothing hard — a small structural move; the only nuance is untangling the ingestion loader (parse/kickoff) from the fileStore ccJson loader without dragging fileStore-source parsing up into `load/`. | a small **structural cleanup** slice | ready now — no prior slice gates it. | 
...
- **Planned (2026-07-04): Slice 15 — full `state/` dissolution** (`slice-15-state-dissolution.md`, the
  ...
  **CF #9** (`isDeltaState`/`areMultipleMapsVisible` → `fileStore/store/` in 15a; `selectedNode`/`hoveredNode`
  → the new `renderModel/` composing layer). When picking those up, use Slice 15 as the home.
