...
|---|------|---------------|------------------|--------------|-------------------|
| 2 | **Dependency (edge) lens — remaining.** Slice 3 landed the edge-metric **DATA** core (`lenses/dependency`); Slice 9a moved `attributeTypes` (node **and** edge — the metrics lens **transiently** owned the edge side) + `attributeDescriptors` into `state.metricsLensSource`; **Slice 14 (`~~2d~~ DONE)** re-homed the edge side of `attributeTypes` into a new `state.dependencyLensSource` root (the twin of `metricsLensSource`), so the metrics lens now owns only node types and the dependency lens owns edge types (ADR 12). **✅ (a) `fileSettings.edges` — CLOSED by Slice 15e:** the `fileSettings` root was deleted outright (`bf50da63b feat(visualization)!: derive edges + delete the fileSettings root slice`); `CcState` has no `fileSettings` at all anymore (verified against `model/state.model.ts`), and edges are now a pure derived selector (`lenses/dependency/store/edges.selector.ts`), never a stored slice — no render-model home or injectable store was needed after all. **Still to move:** (b) the **injectable `DependencyLensStore`/`EdgesRepo`/facade class** — none built (`grep -rln "DependencyLensStore\|EdgesRepo" app/codeCharta` empty, re-checked 2026-07-15); (c) full **deletion of `metricDataSelector`** — still has 6 live non-spec consumers (`renderer/renderModel/accumulatedData/accumulatedData.selector.ts`, `renderer/renderModel/renderModel.facade.ts`, `renderer/renderModel/accumulatedData/metricData/metricData.selector.ts`, `features/metricsBar/stores/metricsBar.read.store.ts`, `features/scenarios/stores/scenarioDialog.store.ts`, `load/effects/reconcileAfterLoad/reconcileAfterLoad.effect.ts`, re-checked 2026-07-15). | Slice 3 (data core); Slice 9a (source split); ~~Slice 14 (edge attributeTypes)~~; ~~Slice 15e (edges → derived selector, fileSettings deleted)~~ | (b) needs an injectable edge UI consumer; (c) needs item #3-era consumers to migrate. | a later **edge-UI** slice | (b) an injectable edge UI consumer exists and reads the dependency lens directly; (c) the 6 listed consumers migrate off `metricDataSelector`. |
...
| 9 | **Cross-cutting read-wrapper dedup (REMAINDER only) — narrowed 2026-07-15.** **✅ Cleanup halves DONE:** 16d folded mapState's last raw `store/*.selector` imports onto `MapStateReadWindow` (added `margin$`, refolded `MarginStore`) so **mapState joined `feature-reaches-state-home-only-via-facade` — all 3 homes fenced**; 16e collapsed the `isDeltaState` ×3 identical wrapper classes into one `features/shared/stores/IsDeltaStateStore`. **✅ `isLoadingFile`/`hoveredNodeId`/`blacklist` — RESOLVED**, verified 2026-07-15 (grep + read): every consumer now goes through the canonical read-window accessor (`fileStoreReadWindow.isLoadingFile$`; `sharedViewReadWindow.hoveredNodeId$`; `sharedViewReadWindow.getBlacklist()` from `3dPrint/stores/stateAccess.store.ts`) — no duplicate wrapper classes found for any of the three. Picked up incidentally by later read-window work (Slice 20 or the fileStore/sharedView read-window builds), not by a dedicated slice. **Still open, narrower than before:** only `selectedNode` — two independent wrappers (`features/metricsBar/stores/metricsBar.read.store.ts:34`, `features/fileExtensionBar/stores/blackListExtension.store.ts:17`) each do their own `store.select(selectedNodeSelector)` instead of sharing one accessor. | Slice 13d; cleanup halves closed by Slice 16d + 16e; isLoadingFile/hoveredNodeId/blacklist closed incidentally by later slices | `selectedNode` feeds selection highlight — **not** snapshot-covered → needs the user's e2e + manual side-by-side vs `main` before folding. | a later **read-window** slice (small — 2 wrappers) | The two `selectedNode` wrappers fold onto one shared accessor; selection-highlight dedup smoked against `main`. |
