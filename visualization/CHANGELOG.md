...
- **Edges no longer disappear on hover/select for zero-value files**: hovering or selecting a building whose height-metric value is `0` (or missing) now still draws its connected edges. Previously `addArrow` gated edge creation on a truthy height-metric value on both endpoints, so such edges showed in the preview but vanished the moment the building was hovered or selected — even though the edge and height metrics are independent.
