...
- **Phone tab still died while a map loaded**: the floor labels draw one map-sized 2D canvas per folder level, sized four times the drawing-buffer width, so even the capped phone buffer produced three canvases of 6128 pixels squared, about 450 MB, plus the same again as textures, and iOS killed the tab before the map appeared. The canvas edge is now capped at 4096 pixels, which also spares a desktop several hundred megabytes.
