...
- **Hide a word**: Right-clicking a word in the cloud now offers **Hide word**, which drops it from the cloud and from the explorer's word list alike — the way to get a noisy word out of the picture without re-running the parser with a `.dlcignore`. A **Hidden Words** segment in the settings bar counts what is hidden and lists it, with a button to show one word again or all of them at once. Hidden words survive a reload like the other domain settings.
- **The cloud and the explorer point at each other**: Clicking a word in the cloud now breaks it down in the explorer's word list instead of doing nothing, and the word the list has expanded is marked in the cloud, so the picture and the list always say the same thing. Clicking the marked word again drops both.
