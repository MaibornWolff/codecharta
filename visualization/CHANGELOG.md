...
- **"M" word-cloud shape**: The word cloud can be laid out inside the MaibornWolff **M** mark, alongside the geometric shapes. It is picked like any other entry in the **Shape** menu and is drawn with a mask silhouette (inline, no asset request), so the words fill the two arches of the M and its proportions are preserved instead of being stretched to the window. Because a mask leaves less usable area than a full circle, the existing "N of M words fit" notice may appear — raise the window size, lower the word spacing, or keep **Fit all words** on.
- **Word cloud screenshot**: The domain view has the metrics view's screenshot capability — a camera button floating above the cloud, plus the same `Ctrl+Alt+S` (save as file) and `Ctrl+Alt+F` (copy to clipboard) hotkeys and the same "screenshot to clipboard" preference. It exports the word cloud alone, rendered by the chart itself at the device pixel ratio, on a transparent background and cropped to the words — no explorer, bars or notices in the image. The hotkeys always act on the view you are looking at, and the button is disabled while a node has no words to show.
- **Domain view explorer find**: The domain view's explorer has a find box that locates a file or folder in the tree — expanding its ancestors, scrolling it into view and briefly highlighting it, with Enter stepping through further matches. The metrics view keeps its map-filtering search; the domain view swaps it for find, since its explorer is the only way to scope the cloud.

### Changed

- **Word cloud honors the Words count**: The word cloud silently left out every word its layout could not place, so large maps showed far fewer words than the **Words** slider promised — with no indication. **Fit all words** is now **on by default**, a new **Draw outside bounds** toggle (off by default) lets words render partially beyond the layout edge instead of being shrunk or dropped, and whenever fewer words than requested still fit, the cloud says so ("212 of 300 words fit …") instead of staying quiet. The **Words** slider now goes up to **1000** (previously 300).
- **Word Sizing controls**: **Min Size** and **Max Size** are now **Smallest Word** and **Largest Word**, which is what they actually set — the least and most important word in the cloud render at exactly those sizes, and every other word is spread linearly between them. A new **Fit all words** toggle decides what happens to a word the layout cannot place: off leaves it out, so the size range is exact but fewer words than the **Words** count appear; on shrinks it until it fits, so the full count is drawn at the cost of tail words rendering below **Smallest Word**.
- **Domain settings bar areas**: The domain bar was restructured to match the metrics bar — instead of one card and a single "Word Cloud Settings" popover, it now shows one segment per area, each with its own cog popover and its own reset: **Shape**, **Word Sizing** (sizing mode, word count, min/max size and word spacing), and **Rotation** (min/max rotation and rotation step).
- **Word cloud tooltip**: Hovering a word now shows the word in bold followed by **Frequency** and **TF-IDF** on separate lines, instead of a single line naming only the metric that currently drives the sizing. The TF-IDF line is omitted for files that carry no scores.
...
- **Explorer is view-agnostic**: The file explorer no longer belongs to the 3D map. What a row means — whether it can be selected, what hovering it shows, what right-clicking it offers, what its trailing column says — is now supplied by the view that mounts the explorer, and the explorer itself only owns the tree, search, sort, collapse, resize and reveal. The metrics view behaves exactly as before; the domain view now gets domain semantics instead of inheriting map ones.
- **Collapsed explorer**: Collapsing now keeps the width you dragged instead of jumping to a fixed one, and the collapsed bar names the selected node's path with a copy button. Collapse state and width both survive a reload. The sort menu opens in the browser's top layer, so it is no longer clipped by the panel.
- **Floating bars clear the explorer**: The domain and metrics settings bars now center in the space beside the explorer panel instead of across the whole window, so their left controls are no longer hidden behind the sidebar on narrow windows.
- **Domain view labels**: The nav bar's view switcher reads **Metrics | Domain** (was **Map**), the word-cloud **Shape** menu shows readable names (Circle, Heart, Diamond, …) instead of the raw layout values, and the smallest-word size floor was raised from 8px to 10px for legibility.

- **Loading spinner is per view**: The full-screen spinner was replaced by one spinner per view. The domain view no longer waits for the 3D map to rebuild, and the map's rebuild is deferred until the metrics view is actually on screen — so switching to the map shows its spinner for exactly as long as that rebuild takes, and working in the domain view is never blocked by a map nobody can see. The nav bar and view switcher stay usable while a view is busy, so a slow view can always be left.

### Fixed 🐞

- **Word cloud renders got slower with every interaction**: Re-rendering the cloud (dragging a settings slider, selecting another node, resizing) never actually cancelled the asynchronous layout already running — each one kept placing all of its remaining words in the background, competing with the live layout for the main thread. After a burst of interactions a render took over 10× its normal time until the zombie layouts drained. Root cause is in `echarts-wordcloud` itself: its abort mechanism dispatches a `CustomEvent` without `cancelable: true`, so the `preventDefault()`-based cancel has been a no-op — the library's entire layout-abort path was dead code. Patched (via patch-package) to make the event cancelable, and to dispose the previous running layout per chart whenever a new one starts, since a not-merge `setOption` replaces the series model the wrapper tracked its layout on. A killed layout now stops within one word placement; measured: a render after 15 interrupted layouts dropped from 19.7 s to 1.3 s (baseline ~1.4 s).
- **Domain view explorer**: Hovering a row in the domain view showed the 3D map's area/height/color metrics; it now shows the node name with its top five domain words, ranked by whichever metric currently drives the cloud's sizing. Right-clicking a row marked it and opened nothing, because the context menu is only mounted in the metrics view; it is now inert there. Files could not be selected at all when the domain view was opened directly (e.g. a `#/domain` link) — selection was gated on a 3D building that view never creates.
- **Collapsing a folder no longer resets the domain cloud**: In the domain view, collapsing a folder in the explorer is now a pure tidy-the-tree action that keeps your selection, instead of clearing it and snapping the cloud back to the whole-map word bank. The metrics view is unchanged.
- **Switching away from the domain view explains itself**: Loading a file with no domain-language data while the domain view is open still returns you to the map view, but now a toast says why ("This file has no domain-language data — switched to the map view") instead of the view silently changing under you.
