...
- **cc.json 2.0 map survives reload and download**: A cc.json **2.0** file loaded into the app is no longer replaced by the sample files on the next page load. Re-exporting a loaded map (for IndexedDB persistence and for the download button) now stamps the flat export shape with the 1.x `apiVersion` it actually is, instead of copying `"2.0"` verbatim onto a 1.x body — which previously made the reloaded/downloaded file fail validation as an outdated major version and fall back to sample files.
