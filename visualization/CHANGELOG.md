...
- **Inspector sidebar**: The attribute sidebar was redesigned as the "Inspector" — a right-side drawer (`features/sidebarInspector/`) that slides in when a building is selected. It shows the selected node's path with a copy-to-clipboard button, the current Area/Height/Color/Edge metric mapping with each metric's global min–max range (including an `inverted` marker for inverted color mapping), and all node metrics with severity-colored bars that visualize each value within its global range (respecting higher-is-better metric directions). Delta values and delta file counts are still shown in comparison mode. The Σ/median attribute-type toggle was removed.
