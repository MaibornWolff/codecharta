...
describe("app.codeCharta.core.multiple", function() {
	...
	describe("multipleService", function() {
		it(
			"instance",
			NGMock.mock.inject(function(multipleFileService) {
				expect(multipleFileService).not.toBe(undefined)
			})
		)

		it(
			"aggregation of two maps",
			NGMock.mock.inject(function(multipleFileService) {
	...
	describe("edges merge", () => {
		...
		it(
			"aggregation of two maps with empty edges should result in empty edges",
			NGMock.mock.inject(function(multipleFileService) {
				file1.edges = []
				file2.edges = []
		...
		it(
			"aggregation of two maps with given edges should result in summarized edges",
			NGMock.mock.inject(function(multipleFileService) {
				file1.edges = edges1
				file2.edges = edges2
		...
		it(
			"aggregation of one map with edges and other without should result in merged edges",
			NGMock.mock.inject(function(multipleFileService) {
	...
	describe("AttributeTypes merge", () => {
		...
		it(
			"aggregation of two maps with different key attributes should result in all keys aggregated map",
			NGMock.mock.inject(function(multipleFileService) {
		...
		it(
			"aggregation of one map with attributes and other without should result in merged with same attributes as the one with them",
			NGMock.mock.inject(function(multipleFileService) {
				file1.attributeTypes = attribute1
