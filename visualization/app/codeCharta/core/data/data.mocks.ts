import { CodeMap, CodeMapNode, Edge, BlacklistType } from "./model/CodeMap"

export const VALID_NODE: CodeMapNode = {
	name: "root",
	attributes: {},
	type: "Folder",
	children: [
		{
			name: "big leaf",
			type: "File",
			attributes: { RLOC: 100, Functions: 10, MCC: 1 },
			link: "http://www.google.de"
		},
		{
			name: "Parent Leaf",
			type: "Folder",
			attributes: {},
			children: [
				{
					name: "small leaf",
					type: "File",
					attributes: { RLOC: 30, Functions: 100, MCC: 100 }
				},
				{
					name: "other small leaf",
					type: "File",
					attributes: { RLOC: 70, Functions: 1000, MCC: 10 }
				}
			]
		}
	]
}

export const VALID_EDGE: Edge[] = [
	{
		fromNodeName: "/root/big leaf",
		toNodeName: "/root/Parent Leaf/small leaf",
		attributes: {
			pairingRate: 89,
			avgCommits: 34
		}
	},
	{
		fromNodeName: "/root/sample1 only leaf",
		toNodeName: "/root/Parent Leaf/small leaf",
		attributes: {
			pairingRate: 89,
			avgCommits: 34
		}
	}
]

export const TEST_FILE_CONTENT = {
	projectName: "Sample Map",
	apiVersion: "1.1",
	nodes: [VALID_NODE]
}

export const TEST_FILE_DATA: CodeMap = {
	fileName: "file",
	projectName: "Sample Map",
	apiVersion: "1.1",
	nodes: VALID_NODE,
	edges: VALID_EDGE
}

export const TEST_DELTA_MAP_A: CodeMap = {
	fileName: "fileA",
	projectName: "Sample Project",
	apiVersion: "1.1",
	nodes: {
		name: "root",
		type: "Folder",
		attributes: {},
		children: [
			{
				name: "big leaf",
				type: "File",
				attributes: { rloc: 100, functions: 10, mcc: 1 },
				link: "http://www.google.de"
			},
			{
				name: "Parent Leaf",
				type: "Folder",
				attributes: {},
				children: [
					{
						name: "small leaf",
						type: "File",
						attributes: { rloc: 30, functions: 100, mcc: 100 }
					},
					{
						name: "other small leaf",
						type: "File",
						attributes: { rloc: 70, functions: 1000, mcc: 10 }
					}
				]
			}
		]
	}
}

export const TEST_MAP_WITH_BLACKLIST: CodeMap = {
	fileName: "fileA",
	projectName: "Sample Project",
	apiVersion: "1.1",
	nodes: {
		name: "root",
		type: "Folder",
		attributes: {},
		children: [
			{
				name: "big leaf",
				type: "File",
				attributes: { rloc: 100, functions: 10, mcc: 1 },
				link: "http://www.google.de"
			},
			{
				name: "Parent Leaf",
				type: "Folder",
				attributes: {},
				children: [
					{
						name: "small leaf",
						type: "File",
						attributes: { rloc: 30, functions: 100, mcc: 100 }
					},
					{
						name: "other small leaf",
						type: "File",
						attributes: { rloc: 70, functions: 1000, mcc: 10 }
					}
				]
			}
		]
	},
	blacklist: [
		{
			path: "/root/Parent Leaf/small leaf",
			type: BlacklistType.exclude
		}
	]
}

export const TEST_MAP_WITH_BLACKLIST_2: CodeMap = JSON.parse(
	JSON.stringify({
		fileName: "sample1.20_3_2019.json",
		projectName: "Sample Project with Edges",
		apiVersion: "1.0",
		nodes: [
			{
				name: "root",
				type: "Folder",
				attributes: {
					unary: 1,
					avgCommits_absolute: 90,
					functions: 1110,
					mcc: 210,
					pairingRate_relative: 157,
					rloc: 500
				},
				children: [
					{
						name: "sample1 only leaf",
						type: "File",
						attributes: {
							rloc: 400,
							functions: 10,
							mcc: 100,
							pairingRate_relative: 32,
							avgCommits_absolute: 17,
							unary: 1
						},
						link: "http://www.google.de",
						origin: "sample1.json",
						path: "/root/sample1 only leaf"
					},
					{
						name: "big leaf",
						type: "File",
						attributes: {
							rloc: 100,
							functions: 10,
							mcc: 1,
							pairingRate_relative: 77,
							avgCommits_absolute: 56,
							unary: 1
						},
						link: "http://www.google.de",
						origin: "sample1.json",
						path: "/root/big leaf"
					},
					{
						name: "Parent Leaf",
						type: "Folder",
						attributes: {
							unary: 1,
							avgCommits_absolute: 73,
							functions: 1100,
							mcc: 110,
							pairingRate_relative: 125,
							rloc: 100
						},
						children: [
							{
								name: "small leaf",
								type: "File",
								attributes: {
									rloc: 30,
									functions: 100,
									mcc: 100,
									pairingRate_relative: 60,
									avgCommits_absolute: 51,
									unary: 1
								},
								origin: "sample1.json",
								path: "/root/Parent Leaf/small leaf"
							},
							{
								name: "other small leaf",
								type: "File",
								attributes: {
									rloc: 70,
									functions: 1000,
									mcc: 10,
									pairingRate_relative: 65,
									avgCommits_absolute: 22,
									unary: 1
								},
								origin: "sample1.json",
								path: "/root/Parent Leaf/other small leaf"
							}
						],
						origin: "sample1.json",
						path: "/root/Parent Leaf"
					}
				],
				origin: "sample1.json",
				path: "/root"
			}
		],
		edges: [
			{
				fromNodeName: "/root/big leaf",
				toNodeName: "/root/Parent Leaf/small leaf",
				attributes: {
					pairingRate: 89,
					avgCommits: 34
				}
			},
			{
				fromNodeName: "/root/sample1 only leaf",
				toNodeName: "/root/Parent Leaf/small leaf",
				attributes: {
					pairingRate: 32,
					avgCommits: 17
				}
			},
			{
				fromNodeName: "/root/Parent Leaf/other small leaf",
				toNodeName: "/root/big leaf",
				attributes: {
					pairingRate: 65,
					avgCommits: 22
				}
			}
		],
		attributeTypes: {
			nodes: [
				{
					rloc: "absolute"
				},
				{
					functions: "absolute"
				},
				{
					mcc: "absolute"
				}
			],
			edges: [
				{
					pairingRate: "relative"
				},
				{
					avgCommits: "absolute"
				}
			]
		},
		blacklist: [
			{
				path: "/root/big leaf",
				type: "exclude"
			}
		]
	})
)

export const TEST_DELTA_MAP_B: CodeMap = {
	fileName: "fileB",
	projectName: "Sample Project",
	apiVersion: "1.1",
	nodes: {
		name: "root",
		type: "Folder",
		attributes: {},
		children: [
			{
				name: "big leaf",
				type: "File",
				attributes: { rloc: 20, functions: 10, mcc: 1 },
				link: "http://www.google.de"
			},
			{
				name: "additional leaf",
				type: "File",
				attributes: { rloc: 10, functions: 11, mcc: 5 },
				link: "http://www.google.de"
			},
			{
				name: "Parent Leaf",
				type: "Folder",
				attributes: {},
				children: [
					{
						name: "small leaf",
						type: "File",
						attributes: { rloc: 30, functions: 100, mcc: 100, more: 20 }
					},
					{
						name: "other small leaf",
						type: "File",
						attributes: { rloc: 70, functions: 1000 }
					},
					{
						name: "big leaf",
						type: "File",
						attributes: { rloc: 20, functions: 10, mcc: 1 },
						link: "http://www.google.de"
					}
				]
			}
		]
	}
}
