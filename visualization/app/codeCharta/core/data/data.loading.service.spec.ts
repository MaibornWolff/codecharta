...
describe("app.codeCharta.core.data.dataLoadingService", function() {
	let dataLoadingService: DataLoadingService, validFileContent
	...
	//noinspection TypeScriptUnresolvedVariable
	beforeEach(
		NGMock.mock.inject(function(_dataLoadingService_) {
	...
	it("should reject null", done => {
		dataLoadingService
			.loadMapFromFileContent("file.json", null, 0)
			.then(
				() => {},
				() => {
					done()
	...
	it("should reject string", done => {
		dataLoadingService
			.loadMapFromFileContent("file.json", "string", 0)
			.then(
				() => {},
				() => {
					done()
	...
	it("should reject or catch invalid file", done => {
		...
		dataLoadingService
			.loadMapFromFileContent("file.json", invalidFileContent, 0)
			.then(
				() => {},
				() => {
					done()
