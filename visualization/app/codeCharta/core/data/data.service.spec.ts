...
describe("app.codeCharta.core.data.dataService", function() {
	...
	//noinspection TypeScriptUnresolvedVariable
	beforeEach(
		NGMock.mock.inject(function(_dataService_) {
	...
	it("subscribe/notify", () => {
		...
		expect(subscriber.onDataChanged).toHaveBeenCalledWith(
			{ metricData: [{ name: "HELLO", maxValue: 0 }], metrics: ["HELLO"], renderMap: null, revisions: [] },
			expect.anything()
		)
	})
	...
	it("process deltas should call deltaCalculator if maps and deltas are set", () => {
		...
		expect(dataService.deltaCalculatorService.provideDeltas).toHaveBeenCalledWith(TEST_DELTA_MAP_A, TEST_DELTA_MAP_B, [
			"functions",
			"mcc",
			"more",
			"rloc",
