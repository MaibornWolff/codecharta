...
export class TreeMapUtils {
	...
	public static buildNodeFrom(
		...
		return {
			...
			parent: parent,
			heightDelta:
				squaredNode.data.deltas && squaredNode.data.deltas[s.heightKey] ? heightScale * squaredNode.data.deltas[s.heightKey] : 0,
	...
	private static nodeHasNoVisibleEdges(squaredNode: SquarifiedValuedCodeMapNode, s: TreeMapSettings): boolean {
		return (
			s.visibleEdges.filter(edge => squaredNode.data.path === edge.fromNodeName || squaredNode.data.path === edge.toNodeName)
				.length == 0
		)
	}

	private static isNodeNonSearched(squaredNode: SquarifiedValuedCodeMapNode, s: TreeMapSettings): boolean {
		return s.searchedNodePaths.filter(path => path == squaredNode.data.path).length == 0
	}
}
