...
import { DomainBarReadWindow } from "./domainBar.readWindow"

describe("DomainBarReadWindow", () => {
    function setup(domainBar = defaultWordCloudSettings) {
        const state: CcState = { ...STATE, domainBar }
        TestBed.configureTestingModule({
            providers: [provideMockStore({ initialState: state }), { provide: State, useValue: { getValue: () => state } }]
        })
        return TestBed.inject(DomainBarReadWindow)
    }

    it("should read the current domain bar settings synchronously", () => {
        // Arrange
        const domainBar = { ...defaultWordCloudSettings, shape: WordCloudShape.diamond }
        const readWindow = setup(domainBar)

        // Act & Assert
        expect(readWindow.getDomainBar()).toEqual(domainBar)
    })
