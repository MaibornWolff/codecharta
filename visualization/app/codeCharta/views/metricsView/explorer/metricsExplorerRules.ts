...
export class MetricsExplorerRules implements ExplorerRules {
    ...
    removeRule(rule: RuleWithCount) {
        if (rule.kind === "METRIC") {
            dispatchRuleChange(this.store, rule.metricRule.type, removeMetricRule({ id: rule.metricRule.id }))
            return
        }
        const removeRuleAction =
            rule.effect === "flatten" ? removeFlattenedNodes({ items: [rule.item] }) : removeExcludedNodes({ items: [rule.item] })
        dispatchRuleChange(this.store, rule.effect, removeRuleAction)
    }

    clearRules(effect: RuleEffect) {
        dispatchRuleChange(this.store, effect, clearRulesOfType({ ruleEffect: effect }))
    }

    ruleFromSearchPattern(effect: RuleEffect) {
        dispatchRuleChange(this.store, effect, ruleFromSearchPattern(effect))
    }
}
