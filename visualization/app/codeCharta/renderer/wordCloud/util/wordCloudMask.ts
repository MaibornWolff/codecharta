/**
 * The MaibornWolff "M" mark as a word-cloud mask. echarts-wordcloud fills the OPAQUE region of a mask image
 * with words, so only the silhouette matters — the two arch paths are filled solid black and the wordmark
 * text and brand colors from the source logo are dropped. The viewBox is cropped to the M's bounding box so
 * the shape fills the canvas rather than sitting in the empty width the full logo lockup occupies.
 */
...
/** The mask as an inline data URI, so no network fetch and no asset-path plumbing is involved. */
export const WORD_CLOUD_M_MASK_DATA_URI = `data:image/svg+xml,${encodeURIComponent(MAIBORNWOLFF_M_MASK_SVG)}`

/**
 * Loads the "M" mask as an image echarts-wordcloud can rasterize. Kept out of the pure option builder,
 * which never touches the DOM; the component loads this once and threads the result into the render context.
 */
