...
describe("node context menu", () => {
	...
	describe("event related behavior", () => {
		...
		it("should broadcast 'show-node-context-menu' when 'show' method is called", () => {
			...
			expect(services.$rootScope.$broadcast).toHaveBeenCalledWith("show-node-context-menu", {
				path: "somepath",
				type: "sometype",
				x: 42,
				y: 24
	...
	describe("showing and hiding", () => {
		...
		it("showing the context menu should should set the correct building after some timeout", () => {
			...
			services.codeMapUtilService.getCodeMapNodeFromPath = jest.fn((path, type) => {
				return { name: path, type: type, attributes: {} }
			})
		...
		it("showing the context menu should set the edge fields after some timeout", () => {
			...
			services.codeMapActionsService.amountOfDependentEdges = jest.fn(() => {
				return 42
			})
