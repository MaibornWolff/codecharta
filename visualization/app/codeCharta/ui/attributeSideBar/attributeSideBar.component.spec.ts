...
describe("AttributeSideBarComponent", () => {
	...
	it("should contain primary/secondary metrics with title/tooltip attribute/metric descriptor information", async () => {
		...
		const selectedNode = klona(TEST_NODE_FOLDER)
		selectedNode["children"] = [{}]
		selectedNode["attributes"] = { a: 3, b: 1, mcc: 4, rloc: 222, someColor: 10 }
		...
		expect(attributeTypeSelectorWithinSecondaryMetrics[1].getAttribute("title")).toBe(metricTitles.get("rloc"))
		expect(attributeTypeSelectorWithinSecondaryMetrics[2].getAttribute("title")).toBe("") // someColor
		expect(attributeTypeSelectorWithinSecondaryMetrics.length).toBe(3) // header + two entries
	})
})
