import { calculateRiskProfile, RiskProfile } from "./riskProfileHelper"
...
describe("riskProfileHelper", () => {
	it("should calculate risk profile for java files", () => {
		...
		for (const node of nodes) {
			calculateRiskProfile(node, actualRiskProfile, fileExtension)
		}

		expect(actualRiskProfile).toEqual({ lowRisk: 1, moderateRisk: 10, highRisk: 100, veryHighRisk: 1000 })
	})
	...
	it("should calculate risk profile for all occurring programming languages in files", () => {
		...
		const nodes: CodeMapNode[] = [
			{ name: "javaFile1", type: NodeType.FILE, attributes: { rloc: 1, mcc: metricThresholdsByLanguage.java["mcc"].percentile70 } },
			{ name: "javaFile2", type: NodeType.FILE, attributes: { rloc: 10, mcc: metricThresholdsByLanguage.java["mcc"].percentile80 } },
			{ name: "javaFile3", type: NodeType.FILE, attributes: { rloc: 100, mcc: metricThresholdsByLanguage.java["mcc"].percentile90 } },
			{
				name: "tsFile",
				type: NodeType.FILE,
