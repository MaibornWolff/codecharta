...
import { KindOfMap, Range, Settings, SettingsService, SettingsServiceSubscriber } from "../../core/settings/settings.service"
import $ from "jquery"
...
export class LegendPanelController implements DataServiceSubscriber, SettingsServiceSubscriber {
	...
	private setMarkingPackagesIntoLegend() {
		...
		if (this.settingsService.settings.map) {
			...
			hierarchy<CodeMapNode>(rootNode)
				.descendants()
	...
	private legendContainsMarkingPackages() {
		return this.markingPackages && this.markingPackages.length > 0 && this.markingPackages != []
	}
	...
	encodeHex(s: string): string {
		...
		return this.encodeRGB(parseInt(s[0] + s[1], 16), parseInt(s[2] + s[3], 16), parseInt(s[4] + s[5], 16))
	}
