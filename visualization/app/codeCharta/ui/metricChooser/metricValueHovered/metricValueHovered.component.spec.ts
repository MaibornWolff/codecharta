...
import { MetricValueHoveredModule } from "./metricValueHovered.module"
...
describe("metricValueHoveredComponent", () => {
	beforeEach(() => {
		TestBed.configureTestingModule({
			imports: [MetricValueHoveredModule]
		})
	})

	it("should display nothing when there is no hovered node", async () => {
		mockedHoveredNodeSelector.mockImplementation(() => undefined)
		const { container } = await render(MetricValueHoveredComponent, {
	...
	it("should display attribute value", async () => {
		...
		await render(MetricValueHoveredComponent, {
			excludeComponentDeclaration: true,
			componentProperties: { metricFor: "areaMetric" }
		})
