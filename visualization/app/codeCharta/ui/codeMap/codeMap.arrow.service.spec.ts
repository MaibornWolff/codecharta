...
describe("CodeMapArrowService", () => {
	...
	it("should add arrows only from origin when there are resolvable edges", () => {
		codeMapArrowService.addEdgeArrowsFromOrigin(
			sampleLeaf,
			[sampleLeaf, sampleLeaf2],
			[{ fromNodeName: "/sample", toNodeName: "/sample2" }, { fromNodeName: "/sample2", toNodeName: "/sample2" }],
			sampleRenderSettings
		)
	...
	it("should add arrows when there are resolvable edges", () => {
		codeMapArrowService.addEdgeArrows(
			[sampleLeaf, sampleLeaf2],
			[{ fromNodeName: "/sample", toNodeName: "/sample2" }],
			sampleRenderSettings
		)
		expect(codeMapArrowService.arrows.length).toBe(1)
	})
