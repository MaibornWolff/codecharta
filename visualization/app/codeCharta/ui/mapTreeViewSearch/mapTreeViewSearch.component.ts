...
export class MapTreeViewSearchController implements SettingsServiceSubscriber, DataServiceSubscriber {
	...
	/* @ngInject */
	constructor(private $timeout: ITimeoutService, private settingsService: SettingsService, private dataService: DataService) {
	...
	onClickBlacklistPattern(blacklistType: BlacklistType) {
		this.settingsService.settings.blacklist.push({ path: this.viewModel.searchPattern, type: blacklistType })
		this.viewModel.searchPattern = ""
		this.onSearchChange()
	}
	...
	private isPatternBlacklisted(blacklistType: BlacklistType) {
		return (
			this.settingsService.settings.blacklist.filter(item => {
