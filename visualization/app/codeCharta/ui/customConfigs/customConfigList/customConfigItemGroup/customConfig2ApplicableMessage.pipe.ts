...
export class CustomConfig2ApplicableMessage implements PipeTransform {
	...
	transform(customConfig: CustomConfigItem): string {
		...
		if (missingMaps.length > 0 && mode.length > 0) {
			return `This view is partially applicable. To complete your view, please switch to the ${mode} mode and select the following map(s): ${missingMaps.join(
				", "
			)}.`
		}
		if (missingMaps.length > 0) {
			return `To fulfill your view, please select the following map(s): ${missingMaps.join(", ")}.`
		}
		if (mode.length > 0) {
