...
export class MetricColorRangeDiagramComponent implements OnChanges {
    ...
    private rangeDiagramContainer() {
        return d3.select(this.elementRef.nativeElement.querySelector("#cc-range-diagram-container"))
    }

    private clearDiagramContainer() {
        this.rangeDiagramContainer().selectAll("*").remove()
    }

    private createSvg() {
        return this.rangeDiagramContainer().append("svg").attr("width", this.svgWidth).attr("height", this.svgHeight)
    }
