...
export type PopoverPositionArea = "top span-right" | "bottom span-right"
...
export class SettingsPopoverShellComponent implements AfterViewInit, OnDestroy {
    ...
    private positionRelativeToAnchor() {
        ...
        if (this.positionArea() === "bottom span-right") {
            placeBelow(popover, anchorRect)
        } else {
            placeAbove(popover, anchorRect)
        }
    }
}

function placeAbove(popover: HTMLElement, anchorRect: DOMRect) {
...
function placeBelow(popover: HTMLElement, anchorRect: DOMRect) {
    popover.style.top = `${anchorRect.bottom}px`
    popover.style.bottom = "auto"
    popover.style.maxHeight = `${Math.max(0, window.innerHeight - anchorRect.bottom - VIEWPORT_PADDING)}px`
}
