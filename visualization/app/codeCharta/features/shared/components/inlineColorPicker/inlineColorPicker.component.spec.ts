...
describe("InlineColorPickerComponent", () => {
    ...
    it("should render an icon trigger instead of the swatch when triggerIcon is set", async () => {
        // Arrange
        const { container } = await render(InlineColorPickerComponent, {
            inputs: { hexColor: "#ff0000", ariaLabel: "Pick custom color", triggerIcon: "fa-solid fa-paintbrush" }
        })
        ...
        // Assert
        expect(container.querySelector("i.fa-paintbrush")).not.toBeNull()
        expect(container.querySelector("color-chrome")).not.toBeNull()
    })
