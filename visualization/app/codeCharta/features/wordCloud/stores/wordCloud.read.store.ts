...
import { wordsForSelectedNodeSelector } from "../../../lenses/domain/domainLens.facade"
...
export class WordCloudReadStore {
    ...
    readonly wordsForSelectedNode$: Observable<DomainWord[]> = this.sharedViewReadWindow.selectedBuildingId$.pipe(
        switchMap(selectedBuildingId => this.store.select(wordsForSelectedNodeSelector(selectedBuildingId)))
    )
