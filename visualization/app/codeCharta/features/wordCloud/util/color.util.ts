...
export function interpolateColor(startColor: string, endColor: string, factor: number): string {
    ...
    const channel = (from: number, to: number) => Math.round(from + (to - from) * clampedFactor)
    return rgbToHex(channel(start.r, end.r), channel(start.g, end.g), channel(start.b, end.b))
}
...
function hexToRgb(hex: string): Rgb {
    const body = HEX_COLOR.test(hex) ? hex.slice(1) : DEFAULT_START_COLOR.slice(1)
    return {
