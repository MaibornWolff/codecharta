...
describe("paddingCalculator", () => {
	describe("calculatePaddingBasedOnBuildingArea", () => {
		...
		it("almost doubling, due to ceiling, child areas and the delta should result in double the previous padding area + 1", () => {
		...
		it("almost doubling (Math.sqrt(4) = 2), due to ceiling, padding should double the previous padding area", () => {
			padding = padding * 2 - 1
			const paddingByArea = calculatePaddingBasedOnBuildingArea(childrenAreaValues, smallestDelta, MIN_BUILDING_AREA, padding)
			expect(paddingByArea).toBe(13)
		})

		it("almost doubling (Math.sqrt(4) = 2), due to ceiling, padding should result in double the previous padding area + 1", () => {
			padding = padding * 2
			const paddingByArea = calculatePaddingBasedOnBuildingArea(childrenAreaValues, smallestDelta, MIN_BUILDING_AREA, padding)
			expect(paddingByArea).toBe(13)
		})
