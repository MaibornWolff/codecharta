...
const PADDING_APPROX_FOR_DEPTH_ZERO = 0.028
const PADDING_APPROX_FOR_DEPTH_ONE = 0.019
		...
		.paddingRight(node => {
			...
			if (rootNode) {
				...
				if (node.depth === 0) {
					...
					return Math.max((rootNode.x1 - rootNode.x0) * PADDING_APPROX_FOR_DEPTH_ZERO, DEFAULT_PADDING_FLOOR_LABEL_FROM_LEVEL_1)
				}
				if (node.depth > 0 && node.depth < 3) {
					return Math.max((rootNode.x1 - rootNode.x0) * PADDING_APPROX_FOR_DEPTH_ONE, DEFAULT_PADDING_FLOOR_LABEL_FROM_LEVEL_2)
				}
			}
