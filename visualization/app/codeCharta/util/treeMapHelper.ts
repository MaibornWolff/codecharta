...
import {SquarifiedTreeMap} from "./treeMapGenerator"
...
export class TreeMapHelper {
    ...
    public static buildRootFolderForFixedFolders(map: CodeMapNode, squarifiedTreeMaps: SquarifiedTreeMap[], heightScale: number, state: State, isDeltaState: boolean): Node {
        const {width, length} = this.getRootFolderDimensions(map, squarifiedTreeMaps)
    ...
    private static getRootFolderDimensions(map: CodeMapNode, squarifiedTreeMaps: SquarifiedTreeMap[]): {width: number, length: number} {
        const folders = clone(map.children)

        folders.sort((a: CodeMapNode, b: CodeMapNode) => {
            if(a.fixed.x + a.fixed.width < b.fixed.x + b.fixed.width) {
                return -1
            }
    ...
    private static nodeHasNoVisibleEdges(codeMapNode: CodeMapNode, s: State): boolean {
        return (
            ...
            s.fileSettings.edges.filter(edge => codeMapNode.path === edge.fromNodeName || codeMapNode.path === edge.toNodeName)
                .length == 0
        )
    }
