import { HierarchyNode, hierarchy } from "d3-hierarchy"
import { CCFile, CodeMapNode } from "../../model/codeCharta.model"
import { FileState } from "../../model/files/files"
import { isLeaf } from "../codeMapHelper"
import { ExcludeMatcher } from "./excludeMatcher"

export const resultsInEmptyMap = (visibleFiles: FileState[], matcher: ExcludeMatcher) => {
    for (const { file } of visibleFiles) {
        if (!resultsInEmptyFile(file, matcher)) {
            return false
        }
    }
    return true
}

const resultsInEmptyFile = (file: CCFile, matcher: ExcludeMatcher) => {
    for (const node of hierarchy(file.map)) {
        if (isNodeIncluded(node, matcher)) {
            return false
        }
    }
    return true
}

const isNodeIncluded = (node: HierarchyNode<CodeMapNode>, matcher: ExcludeMatcher) =>
    isLeaf(node) && node.data.path && !matcher.isExcludedLeaf(node.data.path)
