...
// TODO what kind of methods do we need here?
        ...
        fun isInputAnExistingFile(vararg allowedFiletypes: String): (String) -> Boolean =
        { input ->
            val file = File(input)
        ...
        fun isInputAnExistingFileOrFolder(filetype: String = ""): (String) -> Boolean =
        { input ->
            true
        }

        // not sure if this one is needed, its currently there for testing
        fun isInputBetweenNumbers(
        minValue: Int,
        maxValue: Int,
        ): (String) -> Boolean =
        { input ->
