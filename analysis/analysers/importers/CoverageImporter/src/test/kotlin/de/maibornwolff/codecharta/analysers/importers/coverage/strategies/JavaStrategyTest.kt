...
class JavaStrategyTest {
    ...
    fun `should correctly import coverage report and build project structure`() {
        val expectedOutputPath = "src/test/resources/languages/java/coverage.cc.json"
    ...
    fun `should keep folders surrounding the project when the flag is set`() {
        val projectBuilder = ProjectBuilder()
        val expectedOutputPath = "src/test/resources/languages/java/coverage_full_paths.cc.json"

        JavaStrategy().addNodesToProjectBuilder(File(testReportFilePath), projectBuilder, System.err, true)
