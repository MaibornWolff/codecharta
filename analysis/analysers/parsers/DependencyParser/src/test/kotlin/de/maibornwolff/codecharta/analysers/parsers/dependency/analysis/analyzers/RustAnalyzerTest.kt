...
class RustAnalyzerTest {
    ...
    fun `should prefix the node path with the crate name in a workspace layout`() {
        ...
        // Assert — the crate dir before src ("core") is prepended so cross-crate imports can resolve
    ...
    fun `should resolve crate imports against the crate name so they match cross-crate references`() {
        // Arrange — a file in crate "domain" that references a sibling module via crate::
        val code = """
        ...
        // Assert — the node carries the crate name, and crate:: resolves to [domain, ...] (not dropped),
    ...
    fun `should map regular use imports including glob and alias to dependencies`() {
        // Arrange — regular `use` imports (a `pub use` would be a forwarding node, not a dependency)
        val code = """
    ...
    fun `should create a forwarding node for a pub use re-export`() {
        // Arrange — a crate's lib.rs flattens its public API via pub use
        val code = "pub use workshop::Workshop;"

        // Act
        val report = analyze(code, "/repo/crates/domain/src/lib.rs")

        // Assert — a REEXPORT node domain.Workshop forwards to the real definition domain.workshop.Workshop
    ...
    fun `should resolve a consumer through a flattened pub use re-export without a cycle`() {
        // Arrange — domain defines a self-referencing Workshop in a submodule and re-exports it from
        ...
        // Assert — the carrier node is folded into an alias and dropped; render resolves straight to the
    ...
    fun `should resolve a cross-crate dependency to an internal edge`() {
        // Arrange — a type in crate "domain" used by a free function in crate "adapter" via use domain::...
        ...
        // Act — the cross-file resolver connects the graph
        val resolved = DependencyResolverService.resolveNodes(listOf(domain, adapter))

        // Assert — render's signature use of Workshop becomes an INTERNAL edge to domain.workshop.Workshop
        val render = resolved.first { it.pathWithName.parts.last() == "render" }
        assertThat(render.resolvedNodeDependencies.internalDependencies.map { it.path.withDots() })
    ...
    fun `should resolve crate and self prefixes in pub use re-exports to the crate-rooted target`() {
        // Arrange — a crate root re-exporting via both crate:: and self::
        val code = """
        ...
        // Assert — both forms resolve to the crate-rooted definition path
    ...
    fun `should resolve a super prefix in a pub use re-export from a submodule`() {
        // Arrange — a submodule re-exporting a sibling-of-parent item via super::
        val code = "pub use super::shared::Helper;"

        // Act
        val report = analyze(code, "/repo/crates/domain/src/feature/mod.rs")

        // Assert — fileModulePath [domain, feature]; super:: strips one segment -> [domain, shared, Helper]
    ...
    fun `should not emit a node for a glob pub use re-export`() {
        // Arrange — a wildcard re-export (needs cross-file expansion; out of scope)
        val code = "pub use internal::*;"

        // Act
        val report = analyze(code, "/repo/crates/domain/src/lib.rs")

        // Assert — no forwarding node is synthesized for a glob re-export
        assertThat(report.nodes).isEmpty()
    }

    @Test
    fun `should normalize chained super segments in imports`() {
        // Arrange — a deep module importing via super::super::
        val code = """
        ...
        // Assert — fileModulePath [domain, a, b]; two supers strip two segments -> [domain, shared, Helper]
    ...
    fun `should derive the module path for a file outside any src directory`() {
        // Arrange — a build script with no enclosing src/ directory
        val code = "struct Foo {}"

        // Act
        val report = analyze(code, "build.rs")

        // Assert — falls back to the file name as the module segment, with no crate prefix
    ...
    fun `should derive an empty module path for a blank physical path`() {
        // Arrange — no physical path information at all
        val code = "struct Foo {}"

        // Act
        val report = analyze(code, "")

        // Assert — the node sits at the (empty) crate root
    ...
    fun `should ignore current-directory segments in the physical path`() {
        // Arrange — a path with a leading ./ segment
        val code = "struct Foo {}"

        // Act
        val report = analyze(code, "./crates/domain/src/model.rs")

        // Assert — the "." segment is dropped; crate name and module are derived normally
    ...
    fun `should drop a re-export carrier that has no concrete target`() {
        // Arrange — a synthetic RUST re-export node whose only dependency is a wildcard (no concrete target)
        val carrier = Node(
        ...
        // Assert — with no concrete target to alias onto, the carrier is dropped without error
    ...
    fun `should drop a re-export whose target is not in the analyzed set`() {
        // Arrange — domain re-exports Workshop, but workshop.rs is NOT among the analyzed files
        val domainLib = analyze("pub use workshop::Workshop;", "/repo/crates/domain/src/lib.rs")
        val consumer = analyze(
            """
            use domain::Workshop;
        ...
        // Assert — with no real definition to alias onto, the carrier is dropped and render stays unresolved
        assertThat(resolved.map { it.pathWithName.withDots() }).doesNotContain("domain.Workshop")
        val render = resolved.first { it.pathWithName.parts.last() == "render" }
