...
private val metricGardenerprojectBuilder: MetricGardenerProjectBuilder =
            MetricGardenerProjectBuilder(
                    MetricGardenerNodes(
                            mutableListOf(
                                    MetricGardenerNode(
                                    ...
                                    MetricGardenerNode(
                                            ...
                                            "File",
                                            mapOf(
                                                    "mcc" to 34, "functions" to 8, "classes" to 1,
    ...
    fun whenExtractFileNameFromPathThenSuccess() {
        val privateExtractFileNameFromPathMethod =
                metricGardenerprojectBuilder.javaClass.getDeclaredMethod("extractFileNameFromPath", String::class.java)
        privateExtractFileNameFromPathMethod.isAccessible = true
        val fileName =
                privateExtractFileNameFromPathMethod.invoke(
                        metricGardenerprojectBuilder,
                        "\\test-project\\path1\\test-project.path1.Logic\\Service\\TestService.kt",
                                                           )
    ...
    fun whenGenerateFileNodeThenSuccess() {
        val fileNode =
                metricGardenerprojectBuilder.generateCodeChartaFileNode(
                        metricGardenerprojectBuilder.metricGardenerNodes.metricGardenerNodes.elementAt(0),
                                                                       )
        val node =
                MutableNode(
                        "TestService.kt", NodeType.File,
                        mapOf(
                                "mcc" to 3, "functions" to 3, "classes" to 1, "lines_of_code" to 79,
                                "comment_lines" to 32, "real_lines_of_code" to 40,
                             ),
