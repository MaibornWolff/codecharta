...
@CheckReturnValue
fun <T> assertWithPrintOnFail(detailedMetricTable: DetailedMetricTable,
                              foo: (detailedMetricTable: DetailedMetricTable) -> T): AbstractAssert<*, T> {
    return Assertions.assertThat(foo(detailedMetricTable))
            .describedAs("\n" + detailedMetricToTable(detailedMetricTable) + "\n")
}

@CheckReturnValue
fun <T> assertWithPrintOnFail(overviewMetric: OverviewMetric,
                              foo: (overviewMetric: OverviewMetric) -> T): AbstractAssert<*, T> {
