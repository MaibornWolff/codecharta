 ...
 */
class NodeMaxAttributeMerger(
    var mergeChildrenList: Boolean = false
) : NodeMergerStrategy {
    ...
    private fun createLink(nodes: List<MutableNode>) = nodes.map {
        it.link
    }.firstOrNull {
        it != null && !it.isBlank()
    }

    private fun createAttributes(nodes: List<MutableNode>) = nodes.map {
        it.attributes
